...
type FavoritesScreenProp = NativeStackNavigationProp<RootStackParamList, 'Favorites'>;
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      try {
        ...
        setFavorites(data.map((item: any) => ({ ...item, id: item._id })));
        setLoading(false);
      } catch {
      ...
      <View style={styles.overlay}>
        ...
        {!loading && filtered.length === 0 && !error ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyDesc}>Browse recipes and tap ⭐ to save them here</Text>
          </View>
        ) : (
          <FlatList
            data={filtered}
...
const styles = StyleSheet.create({
  ...
  heartIcon: { fontSize: 20, paddingHorizontal: 14 },
